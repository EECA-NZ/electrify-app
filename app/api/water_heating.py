...
def calculate_water_heating(data: WaterHeatingModel):
    """
    Calculate the energy required for heating water.
    """
    try:
...
def perform_water_heating_calculation(data: WaterHeatingModel):
    ...
    Formula: energy (kWh) = volume (liters) *
        temp increase (C) *
        0.001163 (kWh per liter per degree C)
    """
    energy_required = (
        data.volume_litres * data.temp_increase_celsius * 0.001163 / data.efficiency
    )
